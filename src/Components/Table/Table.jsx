	...
	return (
		<div className='p-6 bg-gradient-to-br from-[#1E1E2F] to-[#141427] min-h-screen text-white flex justify-center items-start w-[100%]'>
			<div className='w-full '>
				<h2 className='text-2xl font-semibold mb-6 text-center text-[#7B61FF]'>
					USER TODO
				</h2>
					...
					<tbody>
						{data?.map((user, index) => {
							return (
								<tr
									key={index}
									className='border-t border-gray-700 hover:bg-[#29293d] transition-all'
								>
									...
									<td>
										...
										<button
											...
											onClick={async () => {
												try {
													await axios.delete(
                            `https://to-dos-api.softclub.tj/api/to-dos?id=${user.id}`
